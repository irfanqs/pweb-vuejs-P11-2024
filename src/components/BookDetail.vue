<template>
	<div class="flex justify-center items-center bg-gray-100">
		<div class="w-full px-10 bg-white rounded-lg shadow-lg">
			...
			<div v-if="book" class="grid grid-cols-1 md:grid-cols-3 gap-6">
				<!-- Book Image -->
				<img :src="book.coverImage || 'path/to/default-image.jpg'" alt="Book Cover" class="col-span-1 w-52 h-52 object-cover rounded-lg shadow-md" />
				...
				<div class="col-span-2 space-y-2">
					...
					<button class="bg-green-500 text-white px-4 py-2 mr-4 rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500" :disabled="book.qty <= 0" @click="borrowBook">Borrow</button>
					<button class="bg-yellow-500 text-white px-4 py-2 rounded-md hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-yellow-500" @click="returnBook">Return</button>
				</div>

				<div class="col-span-3 flex space-x-4"></div>
			</div>
		</div>
	</div>
</template>
